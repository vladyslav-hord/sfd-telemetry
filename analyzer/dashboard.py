...
def build_dashboard(report_directory: str, report_date: str | None = None, telemetry_database: str | None = None, episode_id: int | None = None) -> Path:
    ...
    for path in reports:
        ...
        environment_summary = {key: environment.get(key, {} if key not in {"motifs", "object_usage", "interaction_graph", "barrel_boost_candidates"} else []) for key in ("lifecycle", "object_categories", "interactions_by_type", "object_usage", "interaction_graph", "motifs")}
